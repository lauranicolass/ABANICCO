...
for i=1:length(subTables)
    ...
    for j=1:height(tb1)
        ...
        if tb1.Val1(j)<1.0
            tb3.Th(j)=j.*13;
            tb3.R(j)=1;
            tb3.Per(j)=10*tb1.Val2(j); %% problem here when some shades are 100% that category of colour
    ...
    if figs==1
        ...
        for k=1:height(tb3)
            ...
            if strcmp(string2,' ')
                text(13,3.7,string1,FontSize=14)
                text(13,7.7,strcat('R=',num2str(tb0.R(k))),FontSize=14)
                text(13,6.3,strcat('Theta=',num2str(tb0.Angle(k))),FontSize=14)

            else
            text(tb3.Th(k),3.7,string1,FontSize=14)
            text(tb3.Th(k),1.7,string2,FontSize=14)
            text(tb3.Th(k),5.5,string3,FontSize=14)

            text(tb3.Th(k),7.7,strcat('R=',num2str(tb0.R(k))),FontSize=14)
            text(tb3.Th(k),6.3,strcat('Theta=',num2str(tb0.Angle(k))),FontSize=14)
            end
